## Initially load data to H2 Database using @PostConstruct
Some scenario, we have to create the required table and load the data to the database which will be used by the Microservice. So, we can annotate with initial data load function with `@PostConstruct` to load data to the database once the service has initialized during the startup.
